# 🎓 APISentinel - 5 Learning Tasks para C# .NET

## 📌 Instruções Gerais

- Cada task é **progressiva em complexidade** (Básico → Intermediário → Avançado)
- Leia o código existente **antes de começar** cada task
- Faça **commits pequenos e descritivos** para cada task
- **Teste suas mudanças** antes de considerar concluída

---

## 🚀 TASK 1: Criar o Model para Páginas (Easy - 1-2 horas)
...
4. **Exemplo de como deveria parecer**:
```csharp
...
- Models em C# .NET
- Atributos de Data Annotation
- Properties e tipos primitivos
- Padrão de nomeação

---

## 🔗 TASK 2: Criar o DTO para Páginas (Easy-Médio - 1-2 horas)
...
## 🔧 TASK 3: Implementar a Interface IPageService (Médio - 2-3 horas)
...
## 💾 TASK 4: Implementar PageService com Supabase (Avançado - 3-4 horas)
...
## 🌐 TASK 5: Criar PageController com Endpoints (Avançado - 3-4 horas)
...
1. **TASK 1** (Model) - Base para tudo
2. **TASK 2** (DTO) - Validação e transferência
3. **TASK 3** (Interface) - Define o contrato
4. **TASK 4** (Service) - Lógica de negócio
5. **TASK 5** (Controller) - Expõe a API

Cada task **depende da anterior**, então não pule passos!
...
```bash
git add Backend/apisentinel_net/Models/PageModel.cs
git commit -m "feat: Add PageModel for page management"

git add Backend/apisentinel_net/DTOs/PageDTO.cs
git commit -m "feat: Add PageDTO with request and response classes"

git add Backend/apisentinel_net/Interface/IPageService.cs
git commit -m "feat: Add IPageService interface"

git add Backend/apisentinel_net/Services/APISENTINEL-DEV/PageService.cs
git commit -m "feat: Implement PageService with Supabase integration"

git add Backend/apisentinel_net/Controllers/PageController.cs
git commit -m "feat: Add PageController with CRUD endpoints"
```
...
**P: Por que usar DTOs e não passar o Model diretamente?**
R: DTOs garantem segurança (não expõem IDs internos), validação (atributos) e separação entre API e banco.

**P: Quando usar async/await?**
R: Sempre que fizer I/O (banco, HTTP, arquivo), use async para não bloquear thread.

**P: Como testar meus endpoints?**
R: Use Postman, Insomnia ou curl. No VS Code instale "REST Client" extension.

**P: E se der erro "DbContext not found"?**
R: Verifique se injetou `IPageService` no `Program.cs`: 
```csharp
...
## 🚀 Próximos Passos (Após completar as 5 tasks)

1. Criar migrations do banco para tabela `pages`
2. Criar tela frontend (React/Next.js) para gerenciar páginas
3. Adicionar autenticação/autorização nos endpoints
4. Implementar paginação nos endpoints list
...
- **Models exemplo**: `Models/ScheduleModel.cs`, `Models/ExecutionLogModel.cs`
- **DTOs exemplo**: `DTOs/ScheduleDTO.cs`, `DTOs/ExecutionLogDTO.cs`
- **Interface exemplo**: `Interface/IScheduleService.cs`
- **Service exemplo**: `Services/APISENTINEL-DEV/ScheduleService.cs`
- **Controller exemplo**: `Controllers/ScheduleController.cs`

**Estude essas classes primeiro antes de começar as tasks!**

---

## ✨ Bom aprendizado! 

Cada task é um bloco de construção fundamental em C# .NET. Ao completar todas as 5, você terá dominado o padrão **Model → DTO → Interface → Service → Controller**, que é a arquitetura padrão do ASP.NET Core! 🚀

Qualquer dúvida durante o desenvolvimento, consulte os arquivos de exemplo no projeto ou abra uma issue no GitHub!

---

**Difficulty**: Task 1-2 são Easy, Task 3 é Medium, Task 4-5 são Advanced
**Estimated Total Time**: 10-15 horas
**Commits**: 5 (um por task)
